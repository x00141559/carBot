...
class CosmosDbStorage {
    ...
    delete(keys) {
        ...
        return this.ensureCollectionExists().then(() => Promise.all(keys.map((k) => new Promise((resolve, reject) => this.client.deleteDocument(documentdb_1.UriFactory.createDocumentUri(this.settings.databaseId, this.settings.collectionId, cosmosDbKeyEscape_1.CosmosDbKeyEscape.escapeKey(k)), options, (err) => err && err.code !== 404 ? reject(err) : resolve()))))) // handle notfound as Ok
            .then(() => {
            return;
        }); // void
    }
    ...
    ensureCollectionExists() {
        if (!this.collectionExists) {
            this.collectionExists = new Promise((resolve) => {
                _semaphore.take(() => {
                    const result = this.collectionExists ? this.collectionExists :
                        getOrCreateDatabase(this.client, this.settings.databaseId, this.databaseCreationRequestOption)
                            .then((databaseLink) => getOrCreateCollection(this.client, databaseLink, this.settings.collectionId, this.documentCollectionCreationRequestOption));
                    _semaphore.leave();
                    resolve(result);
                });
            });
